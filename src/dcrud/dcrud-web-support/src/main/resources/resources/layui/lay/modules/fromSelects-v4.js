        ...
        $(target ? target : 'select[' + NAME + ']').each(function (index, select) {
            var othis = $(select),
                ...
                options = {
                    ...
                    /*validator multiple  后加*/
                    validator: othis.attr('validator') || othis.attr('_validator'),
                    multiple: othis.attr('multiple') || othis.attr('_multiple'),
            ...
            var reElem = $('<div class="' + FORM_SELECT + '" ' + SKIN + '="' + fs.config.skin + '">\n\t\t\t\t\t' +
                '<input class="' + HIDE_INPUT + '" ' +
                'value="" name="' + fs.config.formname + '" lay-verify="' + fs.config.layverify + '" ' +
                'lay-verType="' + fs.config.layverType + '" ' +
                (fs.config.validator ? " validator='" + fs.config.validator + "' " : '') +
                (fs.config.multiple ? " multiple='" + fs.config.multiple + "' " : '') +
                ...
                '</div>\n\t\t\t\t\t\t' +
                '<div class="' + TDIV + '">\n\t\t\t\t\t\t\t' +
                '<input type="text" autocomplete="off" placeholder="' + fs.config.placeholder + '" readonly="readonly" unselectable="on" class="' + FORM_INPUT + ' xm-select-input2">\n\t\t\t\t\t\t' +
                '</div>\n\t\t\t\t\t\t' +
                '<div></div>' +
                '\n\t\t\t\t\t' +
                '</div>\n\t\t\t\t\t' +
            ...
            $parent.find("input").blur(function () {
                $parent.find("input[name]").trigger("change");
            });
            othis.after($parent);
            ...
            othis.removeAttr('multiple') && othis.attr('_multiple', fs.config.multiple);
            othis.removeAttr('validator') && othis.attr('_validator', fs.config.validator);
